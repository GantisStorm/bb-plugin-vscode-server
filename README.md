...
In any thread, open the right panel, choose **Open new tab**, then select **VS Code**. A thread has one VS Code panel; reopening it focuses that panel. The panel opens code-server at the thread environment’s workspace; threads without an environment open at their BB thread-storage folder. It reloads its configuration when opened; use **Reload configuration** after changing the setting while it is already open. In the plugin settings, turn on **Capture local server** to detect code-server or VS Code Server on ports 8080 and 8000 and save the first healthy endpoint. Turn the switch off to clear a server that the plugin captured; it does not clear a manually configured URL. Turn it back on to capture again. When no URL is configured, **Detect local server** in the panel performs the same capture. **Open in Browser** delegates the URL to BB's native Browser, whose tabs remain available across thread navigation.
